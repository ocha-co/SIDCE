	...
	function Timeline(options) {
		...
		*/
			...
			thisLayerID, thisLayerType, thisLayerUrl, thisLayerColor, json_url)
		{
            ...
            // SIDCE
            var cluster_url = protocolUrl + '?z=' +
				myZoom + '&' + this.markerUrlParams(startDate, endDate).join('&');

			if (this.mediaType) cluster_url += '&m=' + this.mediaType;

            // SIDCE add victim filter values to url
            if (this.mediaType == 99){

                var v_f_v = 'i:' + $('#impact_field_id').val();

                cluster_url += '&v_f=' + v_f_v;
            }

			markers = new OpenLayers.Layer.GML(thisLayer,cluster_url ,
				{
