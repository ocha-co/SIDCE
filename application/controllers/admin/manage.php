	...
	{	
	    ...
	    if ($_POST)
	    {
			...
			if ($post->action == 'a')		// Add Action
			{
				...
				$post->add_rules('layer_type','required');
                
                // Valid layer if type = wms
                if ($_POST['layer_type'] == 'wms')   $post->add_rules('layer_layer','required');
	        ...
	        if ($post->validate())
	        {
				...
				{ // Save Action				
					...
					$layer->layer_type = $post->layer_type;
					$layer->layer_layer = $post->layer_layer;
					$layer->save();
