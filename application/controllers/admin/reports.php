...
class Reports_Controller extends Admin_Controller
{
    ...
    */
    ...
    /*
	* Load text-CSV file into incident table
    */
	function upload_sigpad($id = false, $saved = false)
	{
		$this->template->content = new View('admin/reports_upload_sigpad');
        $this->template->content->title = Kohana::lang('ui_main.upload_reports_sigpad');
        $this->template->content->form_error = false;
		
        // setup and initialize form field names
		$form = array ( 'sigpad_file' => '',);
		//	copy the form as errors, so the errors will be stored with keys corresponding to the form field names
		$errors = $form;
		$form_error = FALSE;
		...
		if ($_POST)
		{
            ...
            $this->session->set('submit_count', $this->session->get('submit_count') + 1);

            if ($this->session->get('submit_count') > 1)   $post->go = 1;

			// Validate csv file
			$post->add_rules('sigpad_file', 'required', 'upload::valid', 'upload::type[csv,txt]', 'upload::size[2M]');

			// Test to see if things passed the rule checks
			if ($post->validate())
			{
                $dir_csv = Kohana::config('upload.directory').'/';
                $summary = Incident_Model::process_upload_sigpad(upload::save('sigpad_file',null,$dir_csv), $post->go, $post->date_format);
		        $this->template->content->summary = $summary;
            ...
            $this->session->set('submit_count', 0);

		$this->template->content->errors = $errors;
		$this->template->content->form_error = $form_error;
		$this->template->content->last_upload = Incident_Model::getLastUploadSigpad();

    }
	
    /*
    * Humanitarian Bulletin
    * @param string $start_end Timestamp start|end date of week
    */
	function bulletin($start_end=null,$saved=false)
	{
		$this->template->content = new View('admin/reports_bulletin');
        $this->template->content->title = Kohana::lang('ui_main.humanitarian_bulletin');
        $this->template->content->form_error = false;
		...
		if ($_POST)
		{
			...
			if ($post->validate())
			{
                list($s, $e) = explode('|', $post->week_date);
                
                // Check if exist
                $bulletin = ORM::factory('bulletin')->where("bulletin_start = '$s' AND bulletin_end = '$e'")->find();
                if ($bulletin->loaded)   
                    $form['week_date'] = $post->week_date;
                else
                    $bulletin = new Bulletin_Model();
				
                // SAVE 
                $bulletin->bulletin_description = $post->description;
                $bulletin->bulletin_start = $s;
                $bulletin->bulletin_end = $e;
                $bulletin->save();
				$form_saved = TRUE;
        ...
        $oldest = Incident_Model::get_oldest_report_timestamp();
        $s_e = strtotime('-1 week');
        while ($oldest < $s_e){
            
            $s = strtotime('-2 monday', $s_e);
            $e = strtotime('sunday', $s_e);
            ...
            $week_date_op["$s_v|$e_v"] = "$s_l - $e_l";
            $s_e = strtotime('-1 week', $s_e);
        }

        if (!empty($start_end)){
            $form['week_date'] = $start_end;
            list($s,$e) = explode('|', $start_end);
            $bulletin = ORM::factory('bulletin')->where("bulletin_start = '$s' AND bulletin_end = '$e'")->find();
            if ($bulletin->loaded){
                $form['description'] = $bulletin->bulletin_description;
            }

        }
		$this->template->content->form = $form;
		$this->template->content->week_date_op = $week_date_op;
		$this->template->content->errors = $errors;
		$this->template->content->form_error = $form_error;
		
        // Javascript Header
		$this->template->js = new View('admin/reports_bulletin_js');
