...
class Reports_Controller extends Main_Controller {
	...
	public function index($cluster_id = 0)
	{
        ...
        */
        ...
        $start_date = (empty($_GET['s'])) ? strtotime('-2 month') : $_GET['s'];
        $end_date = (empty($_GET['e'])) ? strtotime('now') : $_GET['e'];
        ...
        // Date filters
        $dropdowns = Incident_Model::getDropdownsDate($start_date, $end_date);
	...
	public function submit($id = false, $saved = false)
	{
		...
		{
			$forms[$custom_forms->id] = array('title' => $custom_forms->form_title, 'desc' => $custom_forms->form_description);
		}
        ...
        foreach($forms as $form_id => $form_name){
            ...
            $form['custom_field'][$form_id] = $this->_get_custom_form_fields($id,$form_id,true);
        }
        /****** SIDCE CUSTOM :: END ********/
        ...
        foreach ($forms as $form_id => $form_name){
            $disp_custom_fields[$form_id] = $this->_get_custom_form_fields($id,$form_id,false);
        }
	...
	public function view($id = false)
	{
	 ...
	 /**
	 * Displays a report at home in modal window
	 * @param boolean $id If id is supplied, a report with that id will be
	 * retrieved.
	 */
	public function view_modal($id = false)
	{
		$this->template->header->this_page = 'reports';
		$this->template->content = new View('reports_view_modal');
		$this->template->header->modal = 1;
        ...
        // Add Impact
        $this->template->content->incident_impact = Incident_Model::get_impact($id);
        
        // Add Aid
        $this->template->content->incident_aid = Incident_Model::get_aid($id);
	 ...
	 * retrieved.
	 */
	public function bulletin($start, $end)
	{

		$this->template->header->this_page = 'reports';
		$this->template->content = new View('reports_bulletin');
		$this->template->header->google_api = TRUE;
        $this->template->footer = false;

        $trend = Bulletin_Model::trend($end);
        $i_cities = Bulletin_Model::i_cities($start,$end);
        $i_cat = Bulletin_Model::i_cat($start,$end);
        $m_cat = Bulletin_Model::i_cat($start,$end,'map');
        $m_cat_id = implode(',',array_keys($m_cat));
        $m_cat = implode(',', Bulletin_Model::i_cat($start,$end,'map'));
        
        $this->template->content->title = Bulletin_Model::title($start,$end);
        $this->template->content->description = Bulletin_Model::description($start,$end);
        $this->template->content->copy = Bulletin_Model::copyright();
		
        $this->template->content->trend = $trend;
        $this->template->content->i_cities = $i_cities;
        $this->template->content->i_cat = $i_cat;
        $this->template->content->pdf_url = "reports/bulletin_pdf/$start/$end";

		// Javascript Header
		$this->template->header->map_enabled = TRUE;
	
        $this->template->header->js = new View('reports_bulletin_js');
        $this->template->header->js->trend = $trend;
        $this->template->header->js->i_cities = $i_cities;
        $this->template->header->js->i_cat = $i_cat;
        $this->template->header->js->m_cat = $m_cat;
        $this->template->header->js->m_cat_id = $m_cat_id;
        $this->template->header->js->start = $start;
        $this->template->header->js->end = $end;
		//$this->template->header->js->default_map = Kohana::config('settings.default_map');
		//$this->template->header->js->default_zoom = Kohana::config('settings.default_zoom');
	 ...
	 * @param string $end End date, yyyy-mm-dd
	 */
	public function bulletin_pdf($start, $end)
	{
        
        require Kohana::find_file('vendor', 'html2pdf/html2pdf.class', $required = TRUE);
        require Kohana::find_file('vendor', 'googleChart', $required = TRUE);
		$this->template->content = new View('reports_bulletin_pdf');

        $html2pdf = new HTML2PDF('L','Letter','en');

        $this->template->content->html2pdf = $html2pdf;
        $this->template->content->title = Bulletin_Model::title($start,$end);
        $this->template->content->copy = Bulletin_Model::copyright();
        $this->template->content->description = Bulletin_Model::description($start,$end);
        $this->template->content->filename = Kohana::lang('ui_main.humanitarian_bulletin').'_'.$start.'-'.$end;
        
        //Map
        $img_map = Bulletin_Model::map($start, $end);
        $this->template->content->img_map = $img_map;

        //Cities
        $img_cities = Bulletin_Model::img_cities($start, $end);
        $this->template->content->img_cities = $img_cities;
        
        //Cats
        $img_cat = Bulletin_Model::img_cat($start, $end);
        $this->template->content->img_cat = $img_cat;

        // Trend
        $img_trend = Bulletin_Model::img_trend($end);
        $this->template->content->img_trend = $img_trend;
     ...
     /**
	 * Xls Export
     * @param string $s Start date timestamp
     * @param string $e End date timestamp
	 */
	public function xls($s, $e)
	{
        $db = new Database;
        $excel = new Excel;
		...
		$this->template->content = new View('reports_xls');
        
        $cond = " AND incident_date BETWEEN '".date("Y-m-d H:i:s", $s)."' AND '".date("Y-m-d H:i:s", $e)."'";

		$incidents = $db->query("SELECT DISTINCT i.*, l.`location_name` FROM `".$this->table_prefix."incident` AS i JOIN `".$this->table_prefix."incident_category` AS ic ON (i.`id` = ic.`incident_id`) 
                                JOIN `".$this->table_prefix."category` AS c ON (c.`id` = ic.`category_id`) JOIN `".$this->table_prefix."location` AS l ON (i.`location_id` = l.`id`) 
                                WHERE `incident_active` = '1' $cond ORDER BY incident_date DESC");

			
        $this->template->content->incidents = $incidents;
        $this->template->content->filename = 'Reportes_SIDCE';
        $this->template->content->excel = $excel;
	 ...
	 */
	function _dateFormat($fecha,$format_in,$format_out,$separador='-'){
		
        $mes_corto = Array ("","Ene","Feb","Mar","Abr","May","Jun","Jul","Ago","Sep","Oct","Nov","Dic");
		$mes_largo = array("","Enero","Febrero","Marzo","Abril","Mayo","Junio","Julio","Agosto","Septiembre","Octubre","Novimebre","Diciembre");
		...
		if ($format_in == "aaaa-mm-dd"){
			...
			if ($format_out == "dd-MC-aaaa"){
				$fecha = explode("-",$fecha);
				$fecha_r = $fecha[2]."-".$mes_corto[intval($fecha[1])]."-".$fecha[0];
			}
			
			//05-Julio-2006
			if ($format_out == "dd".$separador."MM".$separador."aaaa"){
				$fecha = explode("$separador",$fecha);
				$fecha_r = $fecha[2].$separador.$mes_largo[intval($fecha[1])].$separador.$fecha[0];
			}
		}
	...
	*/
	...
	private function _get_custom_form_fields($incident_id = false, $form_id = 1, $data_only = false)
	{
		...
		if (!$form_id){
            $first_form = ORM::factory('form')->find_all(1,0);
            $form_id = $first_form[0]->id;
        }
