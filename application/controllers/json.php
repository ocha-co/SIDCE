...
class Json_Controller extends Template_Controller
{
	...
	public function cluster()
    {
        ...
        // SIDCE
        // Count Victims
        $show_impact = (isset($_GET['m'])) ? true : false;

        // Impact filters
        $v_f = (isset($_GET['v_f'])) ? $_GET['v_f'] : ''; 
		...
		while (count($markers))
		{
			...
			foreach ($markers as $key => $target)
			{
				...
				if ($pixels < $distance)
				{
					...
					$cluster[] = $target;
                    $num_impact_c_c += 1*Incident_Model::get_num_impact($target['id'],$v_f);
				}
			}
			...
			if (count($cluster) > 0)
			{
				$cluster[] = $marker;
				$clusters[] = $cluster;
                $num_impact_c[] = $num_impact_c_c + 1*Incident_Model::get_num_impact($marker['id'],$v_f);
			}
			else
			{
				$singles[] = $marker;
                $num_impact_s[] = 1*Incident_Model::get_num_impact($marker['id'],$v_f);
			}
		...
		foreach ($clusters as $c => $cluster)
		{
            ...
            $link_txt =  "$cluster_count ".Kohana::lang('ui_main.reports');
            
            if ($show_impact){
                $cluster_count = $num_impact_c[$c];
                $link_txt = "$cluster_count ".Kohana::lang('ui_main.victims')." ".Kohana::lang('ui_main.in')." ".count($cluster)." ".Kohana::lang('ui_main.reports');
            }

			$json_item = "{";
		    $json_item .= "\"type\":\"Feature\",";
		    $json_item .= "\"properties\": {";
			$json_item .= "\"name\":\"" . str_replace(chr(10), ' ', str_replace(chr(13), ' ', "<a href=" . url::base() . "reports/index/?c=".$category_id."&sw=".$southwest."&ne=".$northeast."&s=".$start_date."&e=".$end_date.">".$link_txt."</a>")) . "\",";			
		    $json_item .= "\"category\":[0], ";
			$json_item .= "\"color\": \"".$color."\", ";
			$json_item .= "\"icon\": \"".$icon."\", ";
		...
		foreach ($singles as $s => $single)
		{
            $count = 1;
            $link_txt =  "$count ".Kohana::lang('ui_main.reports');
            
            if ($show_impact){
                $count = $num_impact_s[$s];
                $link_txt = "$count ".Kohana::lang('ui_main.victims')." ".Kohana::lang('ui_main.in')." 1 ".Kohana::lang('ui_main.reports');
            }

			$json_item = "{";
		    $json_item .= "\"type\":\"Feature\",";
		    $json_item .= "\"properties\": {";
			$json_item .= "\"name\":\"" . str_replace(chr(10), ' ', str_replace(chr(13), ' ', "<a href=" . url::base() . "reports/view/" . $single['id'] . "/>".$link_txt."</a>")) . "\",";	
		    $json_item .= "\"category\":[0], ";
			$json_item .= "\"color\": \"".$color."\", ";
			$json_item .= "\"icon\": \"".$icon."\", ";
		    $json_item .= "\"timestamp\": \"0\", ";
			$json_item .= "\"count\": \"" . $count . "\"";
		    $json_item .= "},";
		    $json_item .= "\"geometry\": {";
		    $json_item .= "\"type\":\"Point\", ";
    ...
    public function category_chart()
	{
        ...
        // get graph data
        $graph_data = array();
        $all_graphs = Incident_Model::get_incidents_to_category_chart($start_date,$end_date,$active,$media_type);
	    echo $all_graphs;
    ...
    * @param string $s string Start Date
    * @param string $e string End Date
    * @param int $cat_id Category ID
    * @return
    */
    public function bulletin($s, $e, $cat_id){

		$this->template = "";
		$this->auto_render = FALSE;
        //header('Content-type: application/json');
		echo Bulletin_Model::json($s, $e, $cat_id);
